...
public ResponseEntity<String> saveTimesheet(@RequestBody TimesheetDTO dto) {
    ...
    for (TimesheetLineDTO lineDTO : dto.getLines()) {
		...
		line.setProjectId(projectRepository.findById(lineDTO.getProjectId())
				.orElseThrow(() -> new RuntimeException("Project Id not found")));
		

       line.setTaskId(taskRepository.findById(lineDTO.getTaskId()).orElseThrow(() -> new RuntimeException("Task Id not found")));
